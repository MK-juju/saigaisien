...
/** 親コンテナのサイズ変更後にLeafletの描画領域を再計算し、地図が消える問題を防ぎます。 */
function ResizeMap() {
  const map = useMap()
  useEffect(() => {
    const container = map.getContainer()
    const resize = () => window.requestAnimationFrame(() => map.invalidateSize({ animate: false }))
