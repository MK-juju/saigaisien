...
export default function Page() {
  ...
  const submitPost = () => { if (role !== '被災者') return showNotice('支援依頼を投稿できるのは被災者アカウントです'); if (!form.title || !form.place || !form.quantity) return showNotice('必要物資・数量・受取場所を入力してください'); setRequests([{id:Date.now(), title:form.title, category:form.category, urgency:form.urgency, place:form.place, time:'今', match:0, body:form.description || '支援を必要としています。', status:'募集中'}, ...requests]); setForm({title:'',category:'水・飲料',quantity:'',place:'',urgency:'中',description:''}); showNotice('支援依頼を公開しました'); setTab('検索') }
  ...
  return <main className={`app-shell ${chromeHidden?'chrome-hidden':''}`}>
    <header className="topbar">
      ...
      <div className="brand"><div className="brand-mark"><HeartHandshake size={22}/></div><div><strong>よりそい</strong><span>災害支援マッチング</span></div></div>
      <div className="location"><MapPin size={15}/>石川県 能登地域</div>
      <div className="top-actions">{isAdmin&&<button className={`level level-${level}`} onClick={()=>setAdminOpen(true)}><span className="status-dot"/>災害レベル Lv.{level}<ChevronRight size={14}/></button>}<IconButton label="通知"><Bell size={19}/><i className="notification-dot"/></IconButton></div>
    </header>
    ...
    <div className="layout">
      <aside className={`sidebar ${menuOpen?'is-open':''}`}><div className="sidebar-head"><div className="side-label">メインメニュー</div></div>{(['検索','投稿','地図','マイページ'] as Tab[]).map((item,i)=>{const I=[Search,Plus,MapPin,UserRound][i]; return <button key={item} className={tab===item?'nav-item active':'nav-item'} onClick={()=>setTab(item)}><I size={19}/>{item}{item==='検索'&&<span className="count">3</span>}</button>})}<div className="side-divider"/><div className="side-label">安全とサポート</div>{isAdmin&&<button className="nav-item" onClick={()=>setAdminOpen(true)}><ShieldCheck size={19}/>管理・安全設定</button>}<button className="nav-item" onClick={()=>showNotice('ヘルプセンターを開きました')}><CircleHelp size={19}/>ヘルプセンター</button><div className="sidebar-footer"><div className="offline"><span className="status-dot green"/>オフライン保存 有効</div><small>通信が不安定な場合も、入力内容は端末に保存されます。</small></div></aside>
      <section className="content"><div className="content-head"><div><p className="eyebrow">{tab==='検索'?'支援を探す':tab==='投稿'?'支援を届ける':tab==='地図'?'地域の状況':'あなたの活動'}</p><h1>{tab}</h1></div>{tab==='検索'&&<button className="primary-button" onClick={()=>setTab('投稿')}><Plus size={17}/>支援を投稿する</button>}</div>
      {tab==='検索'&&<>
        <div className="search-panel"><div className="search-input"><Search size={18}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="物資、地域名などで検索"/><kbd>⌘ K</kbd></div><div className="filters"><SlidersHorizontal size={16}/><span>絞り込み</span>{['すべて','高','中'].map(x=><button key={x} className={urgency===x?'filter active':'filter'} onClick={()=>setUrgency(x)}>{x==='すべて'?'すべての緊急度':`${x}緊急度`}</button>)}</div></div>
        <div className="matching-layout"><section className="latest-requests"><div className="section-title"><div><h2>最新の支援依頼</h2><p>あ��たの近くで、助けを待っている声です</p></div><span>{filtered.length}件</span></div><div className="request-list">{filtered.map(r=><article className="request-card" key={r.id}><div className="card-top"><Badge tone={r.urgency==='高'?'red':r.urgency==='中'?'amber':'blue'}>{r.urgency}緊急度</Badge>{r.status&&<Badge tone={r.status==='募集中'?'green':'neutral'}>{r.status}</Badge>}<span className="card-time"><Clock3 size={14}/>{r.time}</span><IconButton label="ブックマーク" onClick={()=>setRequests(requests.map(x=>x.id===r.id?{...x,saved:!x.saved}:x))}><Bookmark size={18} fill={r.saved?'currentColor':'none'}/></IconButton>{isAdmin&&<IconButton label="投稿を削除" onClick={()=>{if(confirm('この支援依頼を削除しますか？')){setRequests(requests.filter(x=>x.id!==r.id));showNotice('管理者権限で投稿を削除しました')}}}><X size={17}/></IconButton>}</div><h3>{r.title}</h3><p className="card-body">{r.body}</p><div className="card-meta"><span><Package size={15}/>{r.category}</span><span><MapPin size={15}/>{r.place}</span></div><div className="card-bottom"><div className="match"><div className="match-ring"><b>{r.match}%</b></div><span>あなたとの<br/>マッチ度</span></div><button className="text-button" onClick={()=>setSelected(r)}>詳細を見る <ChevronRight size={15}/></button></div></article>)}</div></section><section className={`matching-panel ${level>1?'is-locked':''}`}><div className="matching-head"><div className="matching-title"><HeartHandshake size={18}/><h2>マッチング候補</h2><Badge tone={level>1?'neutral':'green'}>{level>1?'現在利用できません':'70%以上を表示'}</Badge></div><p>{level>1?'災害レベルがLv.1以下になるまでマッチング機能は停止しています。':'距離・緊急度・物資の一致度から、候補を上位に表示しています。'}</p></div>{level<=1?<div className="matching-list">{[{name:'佐藤 花子',item:'飲料水・食料を支援',distance:'2.4km',score:94},{name:'山田 健',item:'水・衛生用品を支援',distance:'5.1km',score:82}].map(c=><div className="matching-row" key={c.name}><div className="avatar small">{c.name.slice(0,1)}</div><div className="matching-person"><b>{c.name}</b><span>{c.item} / {c.distance}</span></div><strong>{c.score}%</strong><button className="text-button" onClick={()=>showNotice(`${c.name}への支援申し出を確認しました`)}>詳細 <ChevronRight size={14}/></button></div>)}</div>:<div className="matching-locked"><LockKeyhole size={22}/><span>現在利用できません</span></div>}</section></div>
      </>}
    ...
    <nav className="mobile-nav">{(['検索','投稿','地図','マイページ'] as Tab[]).map((x,i)=>{const I=[Search,Plus,MapPin,UserRound][i];return <button key={x} className={tab===x?'active':''} onClick={()=>setTab(x)}><I size={20}/><span>{x}</span></button>})}</nav>
...
function MyPage({role,onRoleChange,onNotice}:{role:Role;onRoleChange:(role:Role)=>void;onNotice:(s:string)=>void}) { return <div className="mypage"><div className="profile-card"><div className="avatar">田</div><div><h2>田中 太郎</h2><p>{role} <Badge tone="green">デモアカウント</Badge></p><small>石川県 輪島市</small></div><button className="icon-button" aria-label="プロフィール設定"><Settings size={18}/></button></div><div className="role-switch-card"><div><h3>デモ用アカウント</h3><p>役割を切り替えて、それぞれの画面と権限を確認できます。</p></div><div className="role-switcher">{(['被災者','支援者','管理者'] as Role[]).map(item=><button type="button" key={item} className={role===item?'selected':''} onClick={()=>onRoleChange(item)}>{item}<small>{item==='被災者'?'支援を依頼':item==='支援者'?'物資を支援':'地域・通報を管理'}</small></button>)}</div></div><div className="stats"><div><b>8</b><span>支援した人</span></div><div><b>3</b><span>投稿した依頼</span></div><div><b>5</b><span>保存した依頼</span></div></div><div className="mypage-section"><h3>活動状況</h3><div className="activity"><Package size={19}/><div><b>飲料水の依頼</b><p>支援者とマッチング済み</p></div><Badge tone="green">進行中</Badge><ChevronRight size={16}/></div><div className="activity"><HeartHandshake size={19}/><div><b>毛布を支援しました</b><p>3日前</p></div><Badge tone="neutral">完了</Badge><ChevronRight size={16}/></div></div><div className="mypage-section"><h3>サポート</h3><button className="menu-line" onClick={()=>onNotice('ヘルプセンターを開きました')}><LifeBuoy size={18}/>ヘルプセンター<ChevronRight size={16}/></button><button className="menu-line"><FileText size={18}/>利用規約・プライバシー<ChevronRight size={16}/></button></div></div>}
