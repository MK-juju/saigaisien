...
export default function Page() {
  ...
  // 地域レベルは全ユーザーが確認できます。変更操作だけは管理者モーダルに分離します。
  const [levelOpen, setLevelOpen] = useState(false)
  const [menuOpen, setMenuOpen] = useState(false)
  const [notice, setNotice] = useState('')
  const [draftSaved, setDraftSaved] = useState(false)
  const [form, setForm] = useState({title:'', category:'水・飲料', quantity:'', place:'', urgency:'中', description:'', disaster_type:'地震', tags:''})
  const [chromeHidden, setChromeHidden] = useState(false)

  // ホームと専用ページで同じDBの地域レベルを参照し、初期値への巻き戻りを防ぎます。
  useEffect(() => {
    fetch('/api/regions').then(async (response) => {
      if (!response.ok) return
      const body = await response.json()
      if (Number.isInteger(Number(body.level))) setLevel(Number(body.level))
    }).catch(() => undefined)
  }, [])
  ...
  return <main className={`app-shell ${chromeHidden?'chrome-hidden':''}`}>
    ...
    <div className="layout">
      <aside className={`sidebar ${menuOpen?'is-open':''}`}><div className="sidebar-head"><div className="side-label">メインメニュー</div></div>{(['検索','投稿','地図','マイページ'] as Tab[]).map((item,i)=>{const I=[Search,Plus,MapPin,UserRound][i]; return <button key={item} className={tab===item?'nav-item active':'nav-item'} onClick={()=>setTab(item)}><I size={19}/>{item}{item==='検索'&&<span className="count">3</span>}</button>})}<div className="side-divider"/><a className="nav-item" href="/matching"><HeartHandshake size={19}/>マッチング</a><a className="nav-item" href="/messages/demo"><MessageCircle size={19}/>メールページ</a><div className="side-divider"/><div className="side-label">安全とサポート</div>{isAdmin&&<button className="nav-item" onClick={()=>setAdminOpen(true)}><ShieldCheck size={19}/>管理・安全設定</button>}<button className="nav-item" onClick={()=>showNotice('ヘルプセンターを開きました')}><CircleHelp size={19}/>ヘルプセンター</button><div className="sidebar-footer"><div className="offline"><span className="status-dot green"/>オフライン保存 有効</div><small>通信が不安定な場合も、入力内容は端末に保存されます。</small></div></aside>
